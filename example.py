from client import source_file, ClientError, register_function, create_source
...
source = create_source(name="Example - 1", 
                         url='https://data.cityofchicago.org/resource/x74m-smqb.csv', 
                         timer=86400,
                         description="First of its kind",
                         verifier=verifier_id)
try:
    file = source_file(source['id']) # source_id for the Source, maybe we can add another api which takes source name and give id
